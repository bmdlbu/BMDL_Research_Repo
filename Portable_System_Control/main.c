...
const int diodeWavelengths[3] = {780, 850, 904};
...
/* Sensing Mode Window */
    ...
    sensing_entry = GTK_ENTRY(gtk_builder_get_object(sensing_builder, "sensing_entry"));
...
/* Stored Images Window */
    ...
    int getPosition = maestroGetPosition(fd, 0);
...
void on_sensing_set_save_button_clicked() {

    /* Pausing the process and changing the sleep time */
    sem_wait(&threadLock);
    
    /* Getting the number that was put in the entry */
   entry_rate = gtk_entry_get_text(sensing_entry);

    /* Passing the entry that was read to a (char *) */
    saveRateEntry = (char *)entry_rate;
    
    /* Showing the display rate */
    gtk_label_set_text(sensing_rate_number, saveRateEntry);
    gtk_label_set_text(sensing_current_save_number, saveRateEntry);
    printf("atoi: %d\n", atoi(saveRateEntry)); 
    
    /* Setting the new save rate */
    saveRate = (1.0/atoi(saveRateEntry))*1000000;
    
    /* Clearing the text of the entry */
    gtk_entry_set_text(sensing_entry, "");
...
void on_sensing_vacuum_button_clicked() {
    
    /* Setting the pin HIGH for around 2 seconds */
...
void GrabImage(fc2Context context) {
    ...
    if (saving == 1) {
        ...
        sprintf(filename, "Image_%d_%04d.jpeg", diodeWavelengths[count%3],count);
        ...
        sprintf(saveCommand, "mv Biosensor_%d_%04d.jpeg /media/pi/EXTSTORAGE/Saved_Images", diodeWavelengths[count%3], count);
    ...
    GdkPixbuf* scaledPixBuf = gdk_pixbuf_scale_simple(updatePixBuf, 480,380, GDK_INTERP_BILINEAR);
...
void* imgThreadMain(void* args) {
    ...
    while (operating == 1) {
        ...
        if (capturing == 1) {
            ...
            gtk_label_set_text(sensing_diode_number, diodeWavelengths[laser]);
